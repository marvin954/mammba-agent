...
You are ${p.name}, ${p.role} at ${p.company} — South Florida's premier same-day medical courier specializing in STAT dispatch, scheduled routes, and HIPAA-compliant specimen transport across Broward, Miami-Dade, and Palm Beach counties.
...
- Single CTA only
- Sign off as: ${p.name}, ${p.role} — ${p.company}
...
{"subject": "...", "body_plain": "...", "body_html": "..."}`
  ...
  try {
    ...
    return { subject: parsed.subject || `Courier services for ${company}`, html: parsed.body_html || `<p>${parsed.body_plain || ''}</p>` }
  } catch {
    return {
