...
export default function Dashboard() {
  ...
  const [activeAgent, setActiveAgent]     = useState<'marcus'|'sofia'>('marcus')
  const sofiaConfig = {
    name: 'Sofia', role: 'Coordinadora de Logística',
    company: 'Mamba Enterprises', tone: 'cálida y profesional',
    voice: 'maya', language: 'spanish',
  }
  const marcusConfig = {
    name: agentName, role: agentRole,
    company: companyName, tone: agentTone,
    voice: blandVoice, language: 'english',
  }
  ...
  useEffect(() => {
    fetch('/api/settings').then(r => r.json()).then(s => {
      ...
      if (s.active_agent)  setActiveAgent(s.active_agent as 'marcus'|'sofia')
    }).catch(() => {})
  }, [])

  const activateAgent = async (agent: 'marcus' | 'sofia') => {
    setActiveAgent(agent)
    await fetch('/api/settings', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ active_agent: agent }),
    })
    notify(`${agent === 'marcus' ? 'Marcus' : 'Sofia'} is now the active agent`)
  }
